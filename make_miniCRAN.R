...
pth <- "~/Desktop/GitHub/radiant_miniCRAN"
setwd(pth)
...
# from_github = FALSE
if(pkgs_gh != "") {
  # works IF you put the R-packages compiled from GitHub in the
  # right sub-directories
	pth_gh <- "~/Desktop/GitHub/radiant_miniCRAN/gh"
...
# 	options(repos = c(CRAN = c("http://cran.rstudio.com", mcran_gh)))
	...
	source("makeRepo.R")
	pkgsdir <- "~/Desktop/GitHub/radiant_miniCRAN/gh/pkgs"
...
# rsync -avruz --progress --delete -e ssh bin/ ubu:R/
# rsync -avruz --progress --delete -e ssh src/ ubu:R/
